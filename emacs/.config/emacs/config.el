...
(use-package evil-escape
  ...
  ;; Only enable evil-escape in insert state
  (setq evil-escape-excluded-states '(visual normal replace operator motion emacs))
  (evil-escape-mode 1))
...
(use-package general
  ...
  (dt/leader-keys
    ...
    "m i" '(org-toggle-inline-images :wk "Org toggle item")
...
(define-key global-map (kbd "M-p c") #'compile)
(define-key global-map (kbd "M-p f") #'flutter-run-or-hot-reload)
(define-key global-map (kbd "M-p n") #'my/open-nvim-external)
(define-key global-map (kbd "M-p v") #'vterm-toggle)
(define-key global-map (kbd "M-p i") #'indent-region)
(define-key global-map (kbd "M-p t") #'tab-bar-mode)

(define-key global-map (kbd "M-f") nil)
(define-key global-map (kbd "M-f f") #'my/toggle-fullscreen-buffer)

;; Move between tabs with Alt + Left/Right arrows
...
;; ========= IF YOU ARE USING A PC COMMENT THE LINE =========
...
;; (setq lsp-enable-on-type-formatting t)   ;; format as you type
;; (setq lsp-enable-indentation t)          ;; let LSP handle indentation
...
(use-package yasnippet
  :ensure t
  :hook ((prog-mode . yas-minor-mode)) ;; সব prog-mode এ চালু হবে
  :config
  (yas-reload-all))

;; Optional: ready-made snippet collection
...
(use-package tree-sitter
  ...
  :hook ((prog-mode . tree-sitter-mode)
	 (lsp-mode . tree-sitter-hl-mode)
         (prog-mode. tree-sitter-hl-mode)))
;; Pre-built grammars for many languages
...
(use-package cc-mode
  :ensure nil ;; cc-mode is built into Emacs
...
(defun my/c-mode-setup ()
  "Custom configuration for C / C++ development."
  ;; Disable annoying auto-indent when pressing RET or A
  (electric-indent-local-mode -1)

  ;; Set indentation style and size
  (c-set-style "linux")        ;; choices: "gnu", "k&r", "bsd", "stroustrup", "linux"
  (setq c-basic-offset 2)      ;; number of spaces per indent level
  (setq indent-tabs-mode nil)  ;; use spaces instead of tabs

  ;; Optional: make TAB always indent or complete
  (setq tab-always-indent 'complete)

  ;; Keybinding: format buffer quickly
  (local-set-key (kbd "C-c i") #'indent-region))


;; ============================
;; Optional: Clangd backend
;; ============================
...
;; (defvar my/toggle-maximized-window nil
;;   "Track whether the current window is maximized.")
;; (defvar my/saved-window-config nil
;;   "Store the window configuration before maximizing.")

;; (defun my/toggle-fullscreen-buffer ()
;;   "Toggle the current buffer between full-screen and the previous window configuration."
;;   (interactive)
;;   (if my/toggle-maximized-window
;;       ;; Restore previous window configuration
;;       (progn
;;         (when my/saved-window-config
;;           (set-window-configuration my/saved-window-config))
;;         (setq my/toggle-maximized-window nil))
;;     ;; Maximize current window
;;     (progn
;;       (setq my/saved-window-config (current-window-configuration))
;;       (delete-other-windows)
;;       (setq my/toggle-maximized-window t))))

;; ;; Bind to M-f (Alt-f)
;; (global-set-key (kbd "M-f") 'my/toggle-fullscreen-buffer)


;; (require 'exwm)  ; Ensure EXWM is loaded; add this if not already present
...
(defvar my/saved-frame-params nil
  "Store the frame parameters before maximizing in EXWM.")

(defun my/toggle-fullscreen-buffer ()
  "Toggle the current buffer/window between full-screen and previous state.
For EXWM X clients (e.g., Firefox, Thunar), maximizes the frame to full-screen.
For regular Emacs buffers, maximizes the window within the frame."
  (interactive)
  (if (and (boundp 'exwm-mode) exwm-mode)  ; Check if in an EXWM-managed buffer
      ;; EXWM X window handling: Toggle frame full-screen
      (if my/toggle-maximized-window
          ;; Restore previous frame state
          (progn
            (when my/saved-frame-params
              (modify-frame-parameters nil my/saved-frame-params))
            (setq my/toggle-maximized-window nil)
            (setq my/saved-frame-params nil))
        ;; Maximize frame to full-screen
        (progn
          (setq my/saved-frame-params (frame-parameters))
          ;; Full-screen parameters: fill screen, no decorations, top-left at 0,0
          (modify-frame-parameters nil
                                   '((fullscreen . fullboth)
                                     ...
                                     (tool-bar-lines . 0)))
          (setq my/toggle-maximized-window t)))
    ;; Fallback for regular Emacs buffers: Original logic
    (if my/toggle-maximized-window
        ;; Restore previous window configuration
        (progn
          (when my/saved-window-config
...
(setq exwm-input-global-keys
	...
	([?\M-g] . exwm-layout-toggle-fullscreen)
...
(define-key exwm-mode-map (kbd "M-p") nil)

(define-key exwm-mode-map (kbd "M-p c") #'compile)
(define-key exwm-mode-map (kbd "M-p f") #'flutter-run-or-hot-reload)
(define-key exwm-mode-map (kbd "M-p n") #'my/open-nvim-external)
(define-key exwm-mode-map (kbd "M-p t") #'tab-bar-mode)

(define-key exwm-mode-map (kbd "M-f") nil)
(define-key exwm-mode-map (kbd "M-f f") #'my/toggle-fullscreen-buffer)
(define-key exwm-mode-map (kbd "M-f F") 'find-file)

(define-key exwm-mode-map (kbd "M-<left>") #'tab-previous)
(define-key exwm-mode-map (kbd "M-<right>") #'tab-next)
