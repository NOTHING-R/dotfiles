...
- [[#setting-up-emacs][SETTING UP EMACS]]
  ...
  - [[#window-fullscreen-toggle][WINDOW FULLSCREEN TOGGLE]]
  - [[#emacs-keybinding][EMACS KEYBINDING]]
- [[#setting-up-exwm][SETTING UP EXWM]]
  - [[#exwm][EXWM]]
  - [[#test-for-virt-manager-host-key][Test for virt manager host key]]
  ...
  - [[#test-for-warning-buffer][test for warning buffer]]
...
;; >>>>>Escapeing from the insert mood to normal mode using "jj"<<<<<<
...
(use-package general
  ...
  (dt/leader-keys
    ...
    "f f" '(my/toggle-fullscreen-buffer :wk "Fullscreen Toggle"))
...
(defvar my/exwm-init-reloaded nil)
...
(add-hook
 ...
 (lambda ()
   ...
   ;; ================================
   ;; Reload init.el AFTER 10 sec
   ;; (Only Once - No Loop)
   ;; ================================

   (unless my/exwm-init-reloaded
     (setq my/exwm-init-reloaded t)
     (run-at-time
      "10 sec" nil
      (lambda ()
        (load-file "~/.config/emacs/init.el")
...
(defun my/toggle-fullscreen-buffer ()
  "Toggle current Emacs window between maximized and previous layout."
  (interactive)
...
(define-key global-map (kbd "M-p p") #'vterm-toggle)
(define-key global-map (kbd "M-p v") #'my/launch-vscode)
...
(defun my/power-menu ()
  ...
  (interactive)
  (start-process "wlogout" nil "wlogout"))
...
** Test for virt manager host key
#+begin_src emacs-lisp
(add-hook 'exwm-manage-finish-hook
          (lambda ()
            (when (or (string-match-p "virt-manager" exwm-class-name)
                      (string-match-p "Qemu-system" exwm-class-name)
                      (string-match-p "qemu-system" exwm-class-name))
              (exwm-input-set-local-simulation-keys nil)
...
(add-hook 'exwm-manage-finish-hook
          (lambda ()
            (when (string= exwm-class-name "Virt-manager")
              ;; Always keep keyboard on host
              (exwm-input-release-keyboard)
              (exwm-layout-hide-mode-line))))
...
#+begin_src emacs-lisp
 ...
 (defun my/launch-vscode ()
  "launch firefox browser."
  (interactive)
  (start-process "code" nil "code"))

(defun my/launch-librewolf ()
  "Launch LibreWolf browser (Flatpak)."
  (interactive)
  (start-process
...
(defun my/open-nvim-external ()
 ...
 (defun my/launch-nitrogen ()
  "launch firefox browser."
  (interactive)
  (start-process "nitrogen" nil "nitrogen"))
...
(with-eval-after-load 'exwm
  ...
  (define-key exwm-mode-map (kbd "M-S") #'my/launch-librewolf)
...
(define-key exwm-mode-map (kbd "M-p v") #'my/launch-vscode)
#+end_src
